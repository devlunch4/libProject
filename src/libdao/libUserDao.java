...
// 책이 있는지 없는지와 대여 가능 여부 확인 
	public Map<String, Object> selectUserbook(String rentisbnno, Object password) {
		String sql = "SELECT bookno FROM libbookinfo WHERE bookno = " + rentisbnno + "AND rentyesno = 0";
		return jdbc.selectOne(sql);
	}
	...
	public int userModify() {
		...
		case 2:
			...
			System.out.println("■         xx 도서관 도서관리 프로그램        —  ▢  X ■ ");
			System.out.println("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■ ");
			System.out.println("[회원전용]           전화번호수정                          Ⅰ - Ⅳ      ");
			System.out.println("------------------------------------------");
	...
	public int userModifyAdmin() {
		...
		System.out.println("===위 내용 확인 후 수정할 항목 번호 선택");
		System.out.println("1.이름 \t 2.생년월일 \t 3.주소 \t 4.전화번호 \t 0.변경취소");
	...
	public Map<String, Object> selectrentbook(String inbookisbn, String password) {
		String sql = "SELECT bookno FROM libhistory WHERE bookno = '" + inbookisbn + "'";
		return jdbc.selectOne(sql);
